from domains.ingestion.application.ports.interface.outbound.INewsSource import INewsSource
from domains.ingestion.application.dto.RawArticleDTO import RawArticleDTO
from typing import List

# domains/ingestion/infrastructure/adapters/outbound/NewsApiAdapter.py — Lightweight NewsAPI client
#
...
class NewsFetcher:
    ...
    async def fetch(self, symbol: str, max_results: int = _MAX_HEADLINES) -> list[dict]:
        ...
        if symbol.upper().endswith(".NS") or symbol.upper() in ["NIFTY", "BANKNIFTY", "RELIANCE", "INFY"]:
             query += " India"
...
class NewsApiAdapter(INewsSource):
