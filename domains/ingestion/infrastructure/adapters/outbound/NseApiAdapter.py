from domains.ingestion.application.ports.interface.outbound.IMarketDataSource import IMarketDataSource
from domains.ingestion.application.dto.RawTickDTO import RawTickDTO
from typing import List

# domains/ingestion/infrastructure/adapters/outbound/NseApiAdapter.py — Market price + NSE option chain fetcher
#
...
class OptionChainFetcher:
    ...
    async def _refresh_cookies(self) -> None:
        """Sequential visit to Home then Option Chain page to establish valid session."""
        now = datetime.now(timezone.utc)
        if (
            self._CookieRefreshTime is not None
            and (now - self._CookieRefreshTime).total_seconds() < 300
        ):
            return

        Logger.info("Establishing NSE session (Home -> OptionChain)...")
        try:
            # 1. Visit Home Page
            ...
            async with self._Session.get("https://www.nseindia.com/option-chain", headers=_NSE_HEADERS) as resp:
                self._CookieRefreshTime = now
                cookie_count = len(self._Session.cookie_jar)
                Logger.info("NSE session established (%d cookies)", cookie_count)
    ...
    async def fetch(self, symbol: str) -> Optional[dict]:
        ...
        if symbol.upper() in INDEX_SYMBOLS:
            ...
            clean_name = symbol.upper().replace(".NS", "")
            url = _NSE_OPTION_CHAIN_EQUITY_URL.format(symbol=clean_name)

        try:
            async with self._Session.get(url, headers=_NSE_API_HEADERS) as resp:
                if resp.status != 200:
                ...
                if not raw_data or not raw_data.get("records"):
                    Logger.warning("[%s] NSE API returned empty records, trying yfinance fallback", symbol)
                    return await self._fetch_yfinance_fallback(symbol)
