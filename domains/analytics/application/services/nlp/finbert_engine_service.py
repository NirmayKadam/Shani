...
class FinBertEngineService:
    """
    Singleton — load once, reuse forever.

    Usage:
        engine = FinBertEngineService.get_instance()
        results = await engine.score_batch(["Good quarterly results", "Market crashes"])
    """

    _Instance: Optional['FinBertEngineService'] = None
    ...
    @classmethod
    def get_instance(cls, cache_path: Optional[str] = None) -> 'FinBertEngineService':
