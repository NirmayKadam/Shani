...
class cnn_predictor:
    ...
    def get_macro_features(self, interval="1d", period="2y"):
        vix = yf.Ticker("^VIX").history(period=period, interval=interval)['Close']
        tnx = yf.Ticker("^TNX").history(period=period, interval=interval)['Close']
        dxy = yf.Ticker("DX-Y.NYB").history(period=period, interval=interval)['Close']
    ...
    def predict(self, symbol: str) -> Dict[str, Any]:
        try:
            ...
            df_w = tkr.history(period="5y", interval="1wk")
            df_m = tkr.history(period="10y", interval="1mo")
            
            if df_d.empty: return {"error": f"No data for {symbol}"}

            macro_d = self.get_macro_features("1d", period="2y")
            macro_w = self.get_macro_features("1wk", period="5y").shift(1).dropna()
            macro_m = self.get_macro_features("1mo", period="10y").shift(1).dropna()
